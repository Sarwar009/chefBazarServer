...
app.get("/admin/stats", async (req, res) => {
  try {
    ...
    const pendingOrders = await orderCollection.countDocuments({ orderStatus: "pending" });
    const deliveredOrders = await orderCollection.countDocuments({ orderStatus: "delivered" });
  ...
  console.log({ totalUsers, pendingOrders, deliveredOrders, totalPaymentAmount });
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (!requestStatus) {
        return res.status(400).send({ message: "requestStatus required" });
      }
...
app.post("/orders", async (req, res) => {
  try {
    ...
    if (!orderData.mealId || !orderData.userEmail) {
